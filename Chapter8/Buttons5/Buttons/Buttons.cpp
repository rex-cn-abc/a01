...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_CREATE:
        // 超类化，保存按钮类的原窗口过程，注册新的窗口类
        InitCommonControlsEx(&icc);
        GetClassInfoEx(NULL, TEXT("Button"), &wndclass);

        wndclass.hInstance = ((LPCREATESTRUCT)lParam)->hInstance;
        wndclass.hCursor = LoadCursor(NULL, IDC_CROSS);
        ButtonProcOld = wndclass.lpfnWndProc;
        wndclass.lpfnWndProc = ButtonProc;
        wndclass.lpszClassName = TEXT("MyButton");
        RegisterClassEx(&wndclass);

        // 创建17个子窗口控件
        for (int i = 0; i < NUM; i++)
        {
            hwndButton[i] = CreateWindowEx(0, TEXT("MyButton"), Buttons[i].m_pText,
...
LRESULT CALLBACK ButtonProc(HWND hwndButton, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    // 其他消息传递给默认窗口过程
    return CallWindowProc(ButtonProcOld, hwndButton, uMsg, wParam, lParam);
}